...
- 标准答案和候选关键信息点用于参考，但不要机械要求逐字一致。只需判断 AI 答案是否覆盖了回答当前 query 所必需的信息；与当前 query 无关的补充信息可以忽略。
- 如果文档本身存在多个可行说法、标准答案不唯一，或 AI 答案只给出了其中一个合理方案，不应直接判为事实错误；除非答案明确与证据冲突，或错误排除了其他可行方案。
